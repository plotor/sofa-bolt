 ...
 *
 * ConnectionSelectStrategy 默认实现，采用随机选择，在执行选择连接时大致分为两步：
 *
 * 1. 在开启 CONN_MONITOR_SWITCH 监控时，会从该连接池所有的连接中做一个简单的 filter 操作，把 CONN_SERVICE_STATUS 为 ON 的连接挑选出来，作为选择池。如果没有开启监控，那么选择池就是连接池。
 * 2. 执行挑选策略，获取选择池中的一个连接。
 *
 * @author yunliang.shi
