...
import org.zhenchao.bolt.example.RequestBody;
...
public class SimpleServerUserProcessor extends SyncUserProcessor<RequestBody> {
    ...
    public Object handleRequest(BizContext bizCtx, RequestBody request) throws Exception {
        ...
        Validate.notNull(bizCtx.getConnection());
        Validate.isTrue(bizCtx.getConnection().isFine());

        Long waittime = (Long) bizCtx.getInvokeContext().get(InvokeContext.BOLT_PROCESS_WAIT_TIME);
        Validate.notNull(waittime);
        ...
        Validate.isTrue(RequestBody.class.equals(request.getClass()));
        processTimes(request);
        if (!delaySwitch) {
