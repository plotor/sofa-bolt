 ...
 */
public class SimpleConnectionEventProcessor implements ConnectionEventProcessor {
    ...
    private void doCheckConnection(Connection conn) {
        ...
        Validate.notNull(conn.getPoolKeys());
        Validate.isTrue(conn.getPoolKeys().size() > 0);
        Validate.notNull(conn.getChannel());
        Validate.notNull(conn.getUrl());
        Validate.notNull(conn.getChannel().attr(Connection.CONNECTION).get());
    }
